(function() {
  ...
  function buildProductCard(p) {
    ...
    const specsHtml = [p.storage, p.ram, p.color].filter(Boolean).map(s => `<span class="product-card-spec">${h(s)}</span>`).join('');
    ...
    return `
      <div class="product-card">
        ...
        <div class="product-card-body">
          ...
          <button class="product-card-add" ${p.stock === false ? 'disabled' : `data-product='${JSON.stringify({id:p.id,name:h(p.name),image:p.image,price:p.price,storage:p.storage||'',color:p.color||''}).replace(/'/g, '&#39;')}'`}>
