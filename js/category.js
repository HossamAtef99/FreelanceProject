(function() {
  ...
  const config = {
    phones:      { title: 'Phones',        subtitle: 'Discover the latest smartphones at the best prices.', label: 'Phones',       category: 'phones' },
    tablets:     { title: 'Tablets',       subtitle: 'Discover our range of premium tablets.',        label: 'Tablets',      category: 'tablets' },
    watches:     { title: 'Smart Watches',  subtitle: 'Premium smart watches for every lifestyle.',    label: 'Smart Watches', category: 'watches' },
    earbuds:     { title: 'Earbuds',        subtitle: 'Premium wireless audio for every lifestyle.',   label: 'Earbuds',      category: 'earbuds' },
    chargers:    { title: 'Chargers',       subtitle: 'Fast and reliable charging solutions.',          label: 'Chargers',     category: 'chargers' },
    powerbanks:  { title: 'Power Banks',    subtitle: 'Portable power for your devices on the go.',    label: 'Power Banks',  category: 'powerbanks' },
  };
  ...
  fetch(API_BASE + '/api/products/' + info.category + '?t=' + Date.now())
    .then(r => r.json())
    .then(products => {
      if (!products.length) {
      ...
      grid.innerHTML = products.map(p => {
        ...
        const badgeHtml = p.badge ? `<div class="product-card-badges"><span class="badge badge-new">${h(p.badge)}</span></div>` : '';
        const brandHtml = p.brand ? `<div class="product-card-brand">${h(p.brand)}</div>` : '';
        const specsHtml = (p.storage || p.ram) ? `
          <div class="product-card-specs">
            ${p.storage ? `<span class="product-card-spec">${h(p.storage)}</span>` : ''}
            ${p.ram ? `<span class="product-card-spec">${h(p.ram)} RAM</span>` : ''}
          </div>` : '';
          ...
          <div class="product-card">
            ...
            <div class="product-card-body">
              ...
              <a href="product-detail.html?id=${encodeURIComponent(p.id)}" style="text-decoration:none;color:inherit;">
                <div class="product-card-title">${h(p.name)}</div>
              </a>
              ${specsHtml}
              ${ratingHtml}
              ...
              ${stockHtml}
              <button class="product-card-add add-cart-btn" data-id="${p.id}" data-name="${h(p.name)}" data-price="${p.price}" data-image="${p.image}">
